...
function PageEditorBlock({
  ...
  return (
    <div style={{ marginBottom: 40 }}>
      ...
      >
        <button onClick={() => exec("bold")} style={toolBtn} title="Bold"><b>B</b></button>
        <button onClick={() => exec("italic")} style={toolBtn} title="Italic"><i style={{ fontStyle: "italic" }}>I</i></button>
        <span style={{ width: 1, height: 16, background: BORDER, margin: "0 4px" }} />
        <button onClick={() => exec("justifyLeft")} style={toolBtn} title="Align left">⬱</button>
        <button onClick={() => exec("justifyCenter")} style={toolBtn} title="Center">☰</button>
        <button onClick={() => exec("justifyRight")} style={toolBtn} title="Align right">⬰</button>
        <span style={{ width: 1, height: 16, background: BORDER, margin: "0 4px" }} />
        <select
          ...
          style={{
            ...
            border: `1px solid ${BORDER}`,
            background: "rgba(255,255,255,0.06)",
            color: TEXT, padding: "3px 6px", cursor: "pointer",
          }}
        >
        ...
        <button
          ...
          style={{ ...toolBtn, color: DANGER, marginLeft: "auto" }}
          title="Delete page"
        >
          × Remove
        </button>
...
export default function ChapterEditorPage() {
  ...
  const [chapterNumberEdit, setChapterNumberEdit] = useState("");
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (role !== "creator" && role !== "owner" && session.user.email !== process.env.NEXT_PUBLIC_OWNER_EMAIL) {
        router.replace(`/${locale}/profile`); return;
      }

      const { data: s } = await supabase
        .from("series").select("id, title, project_type, page_size")
        .eq("id", seriesId).eq("user_id", session.user.id).maybeSingle();
      if (!alive) return;

      if (!s) { setLoadError("Series not found or access denied."); setLoadStatus("error"); return; }
      ...
      if (cErr || !c) { setLoadError(cErr?.message || "Chapter not found."); setLoadStatus("error"); return; }
  ...
  async function saveChapterNumber() {
    if (!chapter) return;
    const num = parseInt(chapterNumberEdit, 10);
    if (!Number.isInteger(num) || num <= 0) {
      setChapterNumberEdit(String(chapter.chapter_number));
      return;
    }
    if (num === chapter.chapter_number) return;
    const { error } = await supabase
      .from("chapters").update({ chapter_number: num }).eq("id", chapter.id);
    if (!error) setChapter(prev => prev ? { ...prev, chapter_number: num } : prev);
  }

  async function addPage() {
    if (!chapterId || addingPage) return;
    setAddingPage(true);
  ...
  if (loadStatus === "error" || !chapter) {
    return (
      <main style={{ background: BG, minHeight: "100vh", color: TEXT }}>
        <div style={{ maxWidth: 900, margin: "0 auto", padding: "40px 24px" }}>
          <Link href={backHref} style={{ display: "inline-flex", alignItems: "center", gap: 6, color: MUTED, fontSize: 12, textDecoration: "none", marginBottom: 20 }}>
            ← Back to series
          </Link>
          <div style={{ background: "rgba(201,122,106,0.08)", border: "1px solid rgba(201,122,106,0.2)", borderRadius: 16, padding: "16px 20px", color: DANGER, fontSize: 13 }}>
  ...
  const pageAspect = PAGE_ASPECT[series?.page_size ?? "A4"] ?? (297 / 210);
    ...
    <main style={{ background: BG, minHeight: "100vh", color: TEXT }}>
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "36px 24px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", flexWrap: "wrap", gap: 8, marginBottom: 28, fontSize: 12, color: MUTED }}>
          <Link href={`/${locale}/studio`} style={{ color: MUTED, textDecoration: "none" }}>Studio</Link>
          <span>/</span>
          <Link href={backHref} style={{ color: MUTED, textDecoration: "none" }}>{series?.title || "Series"}</Link>
          <span>/</span>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16, marginBottom: 28, flexWrap: "wrap" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <span style={{ fontSize: 11, fontWeight: 600, padding: "3px 10px", borderRadius: 9999, border: "1px solid rgba(255,255,255,0.1)", background: isPublished ? "rgba(110,168,128,0.1)" : "rgba(255,255,255,0.05)", color: statusColor }}>
              {statusLabel}
            </span>
          </div>
          <Link href={backHref} style={{ fontSize: 12, color: MUTED, textDecoration: "none", padding: "6px 14px", borderRadius: 9999, border: `1px solid ${BORDER}`, background: SURFACE }}>
            ← Back to series
          </Link>
        </div>
        ...
        <div className="grid gap-5 items-start lg:grid-cols-[minmax(0,1fr)_280px]">
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            <div style={{ background: SURFACE, border: `1px solid ${BORDER}`, borderRadius: 16, padding: "20px 24px" }}>
              ...
              <input
                ...
                style={{ width: "100%", background: "rgba(255,255,255,0.04)", border: `1px solid ${BORDER}`, borderRadius: 10, padding: "10px 14px", fontSize: 15, fontWeight: 600, color: TEXT, outline: "none", boxSizing: "border-box" }}
              />
            </div>

            {/* Tab switcher */}
            <div style={{ display: "flex", gap: 4, padding: 4, background: SURFACE, border: `1px solid ${BORDER}`, borderRadius: 12 }}>
              {(["script", "pages"] as const).map(tab => (
                <button
                  key={tab}
                ...
                >
                  {tab === "script" ? "Script / Notes" : `Pages (${pages.length})`}
                </button>
              ))}
            </div>
            ...
            {activeTab === "script" && (
              <div style={{ background: SURFACE, border: `1px solid ${BORDER}`, borderRadius: 16, padding: "20px 24px" }}>
                ...
                <textarea
                  ...
                  rows={28}
                  style={{ width: "100%", background: "rgba(255,255,255,0.04)", border: `1px solid ${BORDER}`, borderRadius: 10, padding: "14px", fontSize: 14, lineHeight: 1.8, color: TEXT, outline: "none", resize: "vertical", boxSizing: "border-box", fontFamily: "inherit" }}
                />
                <div style={{ display: "flex", justifyContent: "flex-end", marginTop: 8 }}>
                  <span style={{ fontSize: 11, color: MUTED }}>{content.length.toLocaleString()} characters</span>
                </div>
              </div>
            )}
                ...
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 24 }}>
                  ...
                  <button
                    ...
                    style={{
                      ...
                      fontSize: 12, fontWeight: 700, cursor: addingPage ? "not-allowed" : "pointer",
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            <div style={{ background: SURFACE, border: `1px solid ${BORDER}`, borderRadius: 16, padding: "20px" }}>
              ...
              {/* Publish toggle */}
              <label style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, cursor: "pointer", padding: "12px 14px", background: "rgba(255,255,255,0.02)", borderRadius: 10, border: `1px solid ${BORDER}`, marginBottom: 14 }}>
                <div>
                  <div style={{ fontSize: 13, fontWeight: 600, color: TEXT }}>{isPublished ? "Published" : "Draft"}</div>
                  <div style={{ fontSize: 11, color: MUTED, marginTop: 2 }}>{isPublished ? "Visible to readers" : "Only you can see this"}</div>
                </div>
                <div
                  onClick={() => setIsPublished(v => !v)}
                  style={{ width: 40, height: 22, borderRadius: 11, background: isPublished ? ACCENT : "rgba(255,255,255,0.1)", position: "relative", transition: "background 0.2s", flexShrink: 0, cursor: "pointer" }}
                >
                  <div style={{ position: "absolute", top: 3, left: isPublished ? 21 : 3, width: 16, height: 16, borderRadius: "50%", background: isPublished ? "#0a0a0c" : "#eceae4", transition: "left 0.2s" }} />
                </div>
              </label>
              ...
              <div style={{ marginBottom: 14 }}>
                ...
                <input
                  ...
                  style={{ width: "100%", background: "rgba(255,255,255,0.04)", border: `1px solid ${BORDER}`, borderRadius: 10, padding: "8px 12px", fontSize: 12, color: TEXT, outline: "none", boxSizing: "border-box", colorScheme: "dark" }}
                />
                {scheduledAt && (
              ...
              {saveError && <p style={{ fontSize: 11, color: DANGER, marginTop: 8 }}>{saveError}</p>}
            </div>
            ...
            <div style={{ background: SURFACE, border: `1px solid ${BORDER}`, borderRadius: 16, padding: "18px 20px" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                ...
                {/* Editable chapter number */}
                <div>
                  <label style={{ fontSize: 11, color: MUTED, display: "block", marginBottom: 4 }}>Chapter #</label>
                  <input
                    type="number"
                    min="1"
                    step="1"
                    value={chapterNumberEdit}
                    onChange={e => setChapterNumberEdit(e.target.value)}
                    onBlur={saveChapterNumber}
                ...
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12 }}>
                  ...
                  <span style={{ color: TEXT, maxWidth: 140, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{series?.title || "—"}</span>
                </div>
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12 }}>
                  <span style={{ color: MUTED }}>Page size</span>
                  <span style={{ color: TEXT }}>{series?.page_size ?? "A4"}</span>
                </div>
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12 }}>
                  <span style={{ color: MUTED }}>Pages</span>
                  <span style={{ color: TEXT }}>{pages.length}</span>
                </div>
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12 }}>
            ...
            {isPublished && (
              <Link
                ...
                style={{ display: "block", textAlign: "center", padding: "10px 0", borderRadius: 9999, border: "1px solid rgba(110,168,128,0.25)", background: "rgba(110,168,128,0.06)", color: SUCCESS, fontSize: 12, fontWeight: 600, textDecoration: "none" }}
              >
                View in reader
              </Link>
            )}
