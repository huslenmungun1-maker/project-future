...
export default function ReaderBookPageView() {
  ...
  const libraryHref = `${baseLocale}/reader/books`;
        ...
        {status === "ok" && page && (
          <div className="flex items-center justify-between gap-3 text-xs">
            <div>
              {prevPageNumber !== null ? (
                <Link
                  href={`${baseLocale}/reader/books/${bookId}/pages/${prevPageNumber}`}
                  className="inline-flex items-center rounded-full border border-slate-700 bg-slate-900/80 px-3 py-1.5 text-xs font-medium text-slate-300 hover:bg-slate-800"
                >
                  ◂ Previous page
                </Link>
            ...
            <div>
              {nextPageNumber !== null ? (
                <Link
                  href={`${baseLocale}/reader/books/${bookId}/pages/${nextPageNumber}`}
                  className="inline-flex items-center rounded-full border border-slate-700 bg-slate-900/80 px-3 py-1.5 text-xs font-medium text-slate-300 hover:bg-slate-800"
                >
                  Next page ▸
                </Link>
