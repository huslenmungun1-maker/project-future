...
create policy "kid_content_self_read" on public.kid_content_submissions
  ...
  using (
    ...
    or exists (select 1 from public.profiles where profiles.user_id = auth.uid() and role = 'owner')
  );
...
create policy "kids_blocked_keywords_owner_read" on public.kids_blocked_keywords
  for select using (
    exists (select 1 from public.profiles where profiles.user_id = auth.uid() and role = 'owner')
  );
